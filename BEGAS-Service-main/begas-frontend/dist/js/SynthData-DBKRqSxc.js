...
import { C as Chart, a as axios, r as registerables } from "./chart-DwolyZdg.js";
...
const _sfc_main = {
  ...
  computed: {
    ...
    syntheticTotalPages() {
      return Math.ceil(Math.min(this.syntheticSamples.length, 50) / this.pageSize) || 1;
    },
    pagedSyntheticSamples() {
      const shown = this.syntheticSamples.slice(0, 50);
      const start = (this.syntheticPage - 1) * this.pageSize;
      return shown.slice(start, start + this.pageSize);
    },
    histogramCandidates() {
      return this.columns.filter((col) => {
        var _a, _b, _c;
        const type = ((_a = this.schema[col]) == null ? void 0 : _a.selectedType) || ((_b = this.schema[col]) == null ? void 0 : _b.inferredType);
        return ((_c = this.schema[col]) == null ? void 0 : _c.include) && type === "Continuous";
      });
    }
  },
  watch: {
    schema: {
      handler(newSchema) {
        for (const col in newSchema) {
          const inferred = newSchema[col].inferredType;
          const selected = newSchema[col].selectedType;
          if (!selected || inferred === selected) continue;
          if (inferred === "Categorical" && selected === "Continuous") {
            alert(`"${col}" 변수는 일반적으로 Categorical(범주형)입니다.
Continuous(연속형)으로 설정 시 잘못된 값이 생성될 수 있습니다.`);
          }
          if (inferred === "Continuous" && selected === "Categorical") {
            alert(`"${col}" 변수는 일반적으로 Continuous(연속형)입니다.
Categorical(범주형)으로 설정 시 잘못된 값이 생성될 수 있습니다.`);
          }
        }
      },
      deep: true
  ...
  methods: {
    onFileChange(e) {
      ...
      this.originalFileName = this.file.name.replace(/\.csv$/, "");
      this.columns = [];
      this.records = [];
      this.schema = {};
      this.syntheticSamples = [];
      this.error = "";
      this.currentPage = 1;
      this.syntheticPage = 1;
    },
    async uploadCsv() {
      if (!this.file) return;
      const form = new FormData();
      form.append("file", this.file);
      form.append("schema", JSON.stringify(this.schema));
      try {
        const res = await axios.post("/synth-data/upload", form);
        const data = res.data;
        ...
        this.columns.forEach((col) => {
          this.schema[col] = {
            include: true,
            inferredType: this.inferType(col),
            selectedType: this.inferType(col)
          };
        });
        this.options.nRows = this.records.length;
      } catch (e) {
        console.error("직접 호출 에러", e);
        this.error = "직접 호출 실패";
      }
    },
    inferType(col) {
      const sample = this.records.slice(0, 10).map((r) => r[col]);
      const numericRatio = sample.filter((v) => !isNaN(parseFloat(v))).length / sample.length;
      return numericRatio > 0.8 ? "Continuous" : "Categorical";
    },
    saveSchema() {
      const filteredCols = this.columns.filter((col) => this.schema[col].include);
      const filteredRows = this.records.map((row) => {
        const newRow = {};
        filteredCols.forEach((col) => {
          const selectedType = this.schema[col].selectedType || this.schema[col].inferredType;
          newRow[col] = selectedType === "Categorical" ? String(row[col]) : row[col];
        });
        return newRow;
      });
      const header = filteredCols;
      const rows = filteredRows.map((row) => header.map((col) => row[col]));
      const csvContent = [header.join(","), ...rows.map((r) => r.map((v) => `"${v}"`).join(","))].join("\r\n");
      const blob = new Blob(["\uFEFF" + csvContent], { type: "text/csv;charset=utf-8;" });
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.setAttribute("download", "setting_data.csv");
    ...
    async generateSynthetic() {
      ...
      try {
        const form = new FormData();
        form.append("records", JSON.stringify(this.records));
        form.append("schema", JSON.stringify(this.schema));
        form.append("n_rows", this.options.nRows);
        form.append("algorithm", this.options.algorithm);
        const interval = setInterval(() => {
          if (this.generationProgress < 90) {
            this.generationProgress += 5;
          }
        }, 500);
        const res = await axios.post("/synth-data/sample", form);
        clearInterval(interval);
        this.generationProgress = 100;
        this.syntheticSamples = res.data;
        this.syntheticPage = 1;
        const statRes = await axios.post("/synth-data/statistics", {
          original: this.records,
          synthetic: this.syntheticSamples,
          schema: this.schema
        });
        this.statisticsResult = statRes.data.statistics;
        if (this.histogramCandidates.length) {
          this.selectedColumn = this.histogramCandidates[0];
          this.$nextTick(() => this.renderHistogram());
        }
      } catch (e) {
        console.error("합성 생성 실패", ((_a = e.response) == null ? void 0 : _a.data) || e);
        this.error = ((_c = (_b = e.response) == null ? void 0 : _b.data) == null ? void 0 : _c.details) || "합성 데이터 생성 실패";
      } finally {
        setTimeout(() => {
    ...
    renderHistogram() {
      ...
      const original = this.records.map((r) => parseFloat(r[col])).filter((v) => !isNaN(v));
      const synthetic = this.syntheticSamples.map((r) => parseFloat(r[col])).filter((v) => !isNaN(v));
      if (!original.length || !synthetic.length) return;
        ...
        this.histChart = new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [
              {
                label: "원본",
                data: this.getHistogramData(original),
                backgroundColor: "rgb(135, 206, 235)"
              },
              {
                label: "합성",
                data: this.getHistogramData(synthetic),
          ...
          options: {
            ...
            scales: {
              x: { title: { display: true, text: "구간", font: { size: 30 } } },
              y: { title: { display: true, text: "빈도수", font: { size: 30 } } }
            }
          }
        });
      });
    ...
    renderDistribution() {
      ...
      const original = this.records.map((r) => parseFloat(r[col])).filter((v) => !isNaN(v));
      const synthetic = this.syntheticSamples.map((r) => parseFloat(r[col])).filter((v) => !isNaN(v));
      const allVals = [...original, ...synthetic];
      ...
      const x = Array.from({ length: n }, (_, i) => Number((min + i * step).toFixed(2)));
      const getDensity = (data) => {
        if (!data.length) return new Array(n).fill(0);
        const dmin = Math.min(...data);
        const dmax = Math.max(...data);
        const bandwidth = 1.06 * Math.sqrt(this.getVariance(data)) * Math.pow(data.length, -1 / 5);
        const gaussian = (x2) => Math.exp(-0.5 * x2 * x2) / Math.sqrt(2 * Math.PI);
        return x.map((xi) => {
          if (xi < dmin || xi > dmax) return 0;
          const sum = data.reduce((acc, val) => acc + gaussian((xi - val) / bandwidth), 0);
          return sum / (data.length * bandwidth);
        });
      };
      const originalDensity = getDensity(original);
      const syntheticDensity = getDensity(synthetic);
      ...
      this.$nextTick(() => {
        ...
        this.distChart = new Chart(ctx, {
          ...
          options: {
            ...
            scales: {
              x: {
                title: { display: true, text: col, font: { size: 30 } },
                ticks: {
                  callback: (v) => Number(v).toFixed(2)
                }
              },
              y: {
                title: { display: true, text: "밀도", font: { size: 30 } }
              }
            }
          }
        });
    ...
    getHistogramData(data) {
      ...
      data.forEach((val) => {
        const i = Math.min(Math.floor((val - min) / step), bins - 1);
        counts[i]++;
      });
      return counts;
    },
    getKernelDensityEstimate(data) {
      ...
      const bandwidth = 1.06 * Math.sqrt(this.getVariance(data)) * Math.pow(data.length, -1 / 5);
      const gaussian = (x2) => Math.exp(-0.5 * x2 * x2) / Math.sqrt(2 * Math.PI);
      const x = Array.from({ length: n }, (_, i) => min + i * step);
      const y = x.map((xi) => {
        const sum = data.reduce((acc, val) => acc + gaussian((xi - val) / bandwidth), 0);
        return sum / (data.length * bandwidth);
      });
    ...
    downloadSyntheticCsv() {
      const fileName = this.originalFileName || "synthetic_data";
      if (!this.syntheticSamples.length) return;
      const header = this.columns;
      const rows = this.syntheticSamples.map(
        (row) => header.map((col) => {
          const val = row[col];
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.setAttribute("download", `${fileName}_syn.csv`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    },
    downloadStatisticsExcel() {
      const wsData = [["컬럼", "평균(원본)", "평균(합성)", "표준편차(원본)", "표준편차(합성)", "최소값(원본)", "최소값(합성)", "최대값(원본)", "최대값(합성)", "IQR(원본)", "IQR(합성)", "p-value"]];
      for (const col in this.statisticsResult) {
        const stat = this.statisticsResult[col];
        wsData.push([col, stat.mean_orig, stat.mean_synth, stat.std_orig, stat.std_synth, stat.min_orig, stat.min_synth, stat.max_orig, stat.max_synth, stat.iqr_orig, stat.iqr_synth, stat.p_value]);
      }
      const ws = utils.aoa_to_sheet(wsData);
      const wb = utils.book_new();
      utils.book_append_sheet(wb, ws, "통계비교");
      const fileName = this.originalFileName || "statistics_result";
      writeFileSync(wb, `${fileName}_stats.xlsx`);
    },
    getVariance(data) {
...
const _hoisted_11 = { class: "raw-table-wrapper" };
const _hoisted_12 = { class: "data-frame" };
const _hoisted_13 = ["onUpdate:modelValue"];
const _hoisted_14 = ["onUpdate:modelValue"];
const _hoisted_15 = { class: "content-box" };
const _hoisted_16 = ["disabled"];
const _hoisted_17 = { class: "content-box" };
const _hoisted_18 = { class: "filter-options" };
...
const _hoisted_22 = { class: "raw-table-wrapper" };
const _hoisted_23 = { class: "raw-table" };
const _hoisted_24 = { class: "pagination-controls" };
...
const _hoisted_32 = { class: "data-frame" };
const _hoisted_33 = {
  key: 6,
  class: "error"
};
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return _openBlock(), _createElementBlock("div", _hoisted_1, [
    _cache[36] || (_cache[36] = _createElementVNode("h2", null, "재현 데이터 생성", -1)),
    _createElementVNode("div", _hoisted_2, [
      ...
      _createElementVNode("button", {
        ...
        disabled: !$data.file,
        onClick: _cache[1] || (_cache[1] = (...args) => $options.uploadCsv && $options.uploadCsv(...args))
      }, "업로드 후 표시", 8, _hoisted_3)
    ]),
    $data.columns.length ? (_openBlock(), _createElementBlock("div", _hoisted_4, [
      _cache[16] || (_cache[16] = _createElementVNode("h2", null, "원본 데이터 미리보기", -1)),
      _createElementVNode("div", _hoisted_5, [
        _createElementVNode("table", _hoisted_6, [
          _createElementVNode("thead", null, [
            _createElementVNode("tr", null, [
              (_openBlock(true), _createElementBlock(_Fragment, null, _renderList($data.columns, (c) => {
                return _openBlock(), _createElementBlock("th", { key: c }, _toDisplayString(c), 1);
              }), 128))
            ])
          ]),
          _createElementVNode("tbody", null, [
            (_openBlock(true), _createElementBlock(_Fragment, null, _renderList($options.pagedRecords, (row, idx) => {
      ...
      _createElementVNode("div", _hoisted_7, [
        ...
        _createElementVNode("span", null, _toDisplayString($data.currentPage) + " / " + _toDisplayString($options.totalPages), 1),
        _createElementVNode("button", {
    ...
    $data.columns.length ? (_openBlock(), _createElementBlock("div", _hoisted_10, [
      _cache[19] || (_cache[19] = _createElementVNode("h2", null, "데이터 설정", -1)),
        ...
        _createElementVNode("table", _hoisted_12, [
          _cache[18] || (_cache[18] = _createElementVNode("thead", null, [
            _createElementVNode("tr", null, [
              ...
              _createElementVNode("th", null, "변수 타입"),
              _createElementVNode("th", null, "변수 타입(선택)")
            ])
          ], -1)),
          _createElementVNode("tbody", null, [
              ...
              return _openBlock(), _createElementBlock("tr", { key: col }, [
                ...
                _createElementVNode("td", null, [
                  _withDirectives(_createElementVNode("input", {
                    type: "checkbox",
                    "onUpdate:modelValue": ($event) => $data.schema[col].include = $event
                  }, null, 8, _hoisted_13), [
                    [_vModelCheckbox, $data.schema[col].include]
                  ])
                ]),
                _createElementVNode("td", null, _toDisplayString($data.schema[col].inferredType), 1),
                _createElementVNode("td", null, [
                  _withDirectives(_createElementVNode("select", {
                    "onUpdate:modelValue": ($event) => $data.schema[col].selectedType = $event
                  }, _cache[17] || (_cache[17] = [
                    _createElementVNode("option", { value: "" }, "(선택)", -1),
                    _createElementVNode("option", { value: "Categorical" }, "Categorical", -1),
                    _createElementVNode("option", { value: "Continuous" }, "Continuous", -1)
                  ]), 8, _hoisted_14), [
                    [_vModelSelect, $data.schema[col].selectedType]
                  ])
                ])
              ]);
            }), 128))
      ...
      _createElementVNode("div", _hoisted_15, [
        _createElementVNode("button", {
          class: "button-primary",
          onClick: _cache[4] || (_cache[4] = (...args) => $options.saveSchema && $options.saveSchema(...args)),
          disabled: !$data.columns.length
        }, "설정 저장 및 다운로드", 8, _hoisted_16)
      ])
    ])) : _createCommentVNode("", true),
    _createElementVNode("div", _hoisted_17, [
      _cache[23] || (_cache[23] = _createElementVNode("h2", null, "생성 옵션 조정", -1)),
      _createElementVNode("div", _hoisted_18, [
        _createElementVNode("label", null, [
          _cache[20] || (_cache[20] = _createTextVNode(" 데이터 생성 수: ")),
          _withDirectives(_createElementVNode("input", {
            type: "number",
            "onUpdate:modelValue": _cache[5] || (_cache[5] = ($event) => $data.options.nRows = $event),
            placeholder: $data.records.length
          }, null, 8, _hoisted_19), [
            [
              _vModelText,
              $data.options.nRows,
        ...
        _createElementVNode("label", null, [
          _cache[22] || (_cache[22] = _createTextVNode(" 알고리즘 ")),
          _withDirectives(_createElementVNode("select", {
            "onUpdate:modelValue": _cache[6] || (_cache[6] = ($event) => $data.options.algorithm = $event)
          }, _cache[21] || (_cache[21] = [
            _createElementVNode("option", { value: "Copula" }, "Copula", -1),
            _createElementVNode("option", { value: "CTGAN" }, "CTGAN", -1),
            _createElementVNode("option", { value: "TVAE" }, "TVAE", -1)
          ]), 512), [
            [_vModelSelect, $data.options.algorithm]
          ])
        ])
      ]),
      _createElementVNode("button", {
        class: "button-primary",
        onClick: _cache[7] || (_cache[7] = (...args) => $options.generateSynthetic && $options.generateSynthetic(...args))
      }, "합성 데이터 생성"),
      $data.isGenerating ? (_openBlock(), _createElementBlock("div", _hoisted_20, "생성 중입니다... " + _toDisplayString($data.generationProgress) + "%", 1)) : _createCommentVNode("", true)
    ]),
    $options.pagedSyntheticSamples.length ? (_openBlock(), _createElementBlock("div", _hoisted_21, [
      _cache[24] || (_cache[24] = _createElementVNode("h2", null, "Synthetic Data Preview Sample", -1)),
      _createElementVNode("div", _hoisted_22, [
        _createElementVNode("table", _hoisted_23, [
          ...
          _createElementVNode("tbody", null, [
            (_openBlock(true), _createElementBlock(_Fragment, null, _renderList($options.pagedSyntheticSamples, (row, idx) => {
              return _openBlock(), _createElementBlock("tr", { key: idx }, [
                (_openBlock(true), _createElementBlock(_Fragment, null, _renderList($data.columns, (col) => {
                  return _openBlock(), _createElementBlock("td", { key: col }, _toDisplayString(row[col]), 1);
                }), 128))
              ]);
            }), 128))
          ])
      ...
      _createElementVNode("div", _hoisted_24, [
        _createElementVNode("button", {
          onClick: _cache[8] || (_cache[8] = (...args) => $options.prevSyntheticPage && $options.prevSyntheticPage(...args)),
          disabled: $data.syntheticPage === 1
        }, "이전", 8, _hoisted_25),
        _createElementVNode("span", null, _toDisplayString($data.syntheticPage) + " / " + _toDisplayString($options.syntheticTotalPages), 1),
        _createElementVNode("button", {
          onClick: _cache[9] || (_cache[9] = (...args) => $options.nextSyntheticPage && $options.nextSyntheticPage(...args)),
          disabled: $data.syntheticPage === $options.syntheticTotalPages
        }, "다음", 8, _hoisted_26)
      ]),
      _createElementVNode("button", {
        class: "button-primary",
        onClick: _cache[10] || (_cache[10] = (...args) => $options.downloadSyntheticCsv && $options.downloadSyntheticCsv(...args))
      }, "재현 데이터 다운로드")
    ])) : _createCommentVNode("", true),
    $options.histogramCandidates.length && $data.syntheticSamples.length ? (_openBlock(), _createElementBlock("div", _hoisted_27, [
      _cache[26] || (_cache[26] = _createElementVNode("h2", null, "분포 비교 (Histogram)", -1)),
      _cache[27] || (_cache[27] = _createElementVNode("p", {
        class: "note",
        style: { "font-size": "18px" }
      }, [
        _createTextVNode(" Histogram은 데이터를 일정 구간(bin)으로 나누어 각 구간에 속하는 데이터의 개수를 시각적으로 표현."),
        _createElementVNode("br"),
        _createTextVNode(" X축은 값의 범위(구간), Y축은 해당 구간에 속한 빈도수를 나타냄."),
        _createElementVNode("br"),
        _createTextVNode(" 예: [0~10), [10~20), ..., [90~100] ")
      ], -1)),
      _createElementVNode("label", null, [
        _cache[25] || (_cache[25] = _createTextVNode(" 항목 선택: ")),
        _withDirectives(_createElementVNode("select", {
          "onUpdate:modelValue": _cache[11] || (_cache[11] = ($event) => $data.selectedColumn = $event),
          onChange: _cache[12] || (_cache[12] = (...args) => $options.renderHistogram && $options.renderHistogram(...args))
        }, [
          (_openBlock(true), _createElementBlock(_Fragment, null, _renderList($options.histogramCandidates, (col) => {
            return _openBlock(), _createElementBlock("option", {
              value: col,
              key: col
            }, _toDisplayString(col), 9, _hoisted_28);
          }), 128))
        ], 544), [
          [_vModelSelect, $data.selectedColumn]
        ])
      ]),
      _cache[28] || (_cache[28] = _createElementVNode("canvas", { id: "histChart" }, null, -1))
    ])) : _createCommentVNode("", true),
    $options.histogramCandidates.length && $data.syntheticSamples.length ? (_openBlock(), _createElementBlock("div", _hoisted_29, [
      _cache[30] || (_cache[30] = _createElementVNode("h2", null, "분포 함수 (Distribution Function)", -1)),
      _cache[31] || (_cache[31] = _createElementVNode("p", {
        class: "note",
        style: { "font-size": "18px" }
      }, [
        _createTextVNode(" 분포 함수(커널 밀도 추정)는 데이터의 전체적인 분포 형태를 부드러운 곡선으로 나타냄."),
        _createElementVNode("br"),
        _createTextVNode(" 원본(하늘색), 합성(핑크색) 데이터를 시각적으로 비교할 수 있다. ")
      ], -1)),
      _createElementVNode("label", null, [
        _cache[29] || (_cache[29] = _createTextVNode(" 항목 선택: ")),
        _withDirectives(_createElementVNode("select", {
          "onUpdate:modelValue": _cache[13] || (_cache[13] = ($event) => $data.selectedColumn = $event),
          onChange: _cache[14] || (_cache[14] = (...args) => $options.renderDistribution && $options.renderDistribution(...args))
        }, [
          (_openBlock(true), _createElementBlock(_Fragment, null, _renderList($options.histogramCandidates, (col) => {
            return _openBlock(), _createElementBlock("option", {
              value: col,
              key: col
            }, _toDisplayString(col), 9, _hoisted_30);
          }), 128))
        ], 544), [
          [_vModelSelect, $data.selectedColumn]
        ])
      ]),
      _cache[32] || (_cache[32] = _createElementVNode("canvas", { id: "distChart" }, null, -1))
    ])) : _createCommentVNode("", true),
    Object.keys($data.statisticsResult).length ? (_openBlock(), _createElementBlock("div", _hoisted_31, [
      _cache[34] || (_cache[34] = _createElementVNode("h2", null, "통계 비교 결과 (원본/합성)", -1)),
      _cache[35] || (_cache[35] = _createElementVNode("p", {
        class: "note",
        style: { "font-size": "18px" }
      }, [
        _createTextVNode(" 표준 편차 : 표준편차는 데이터가 평균값을 중심으로 얼마나 퍼져 있는지를 나타내는 지표. 값이 작을수록 평균에 가까운 데이터가 많고, 값이 클수록 평균에서 멀리 떨어진 데이터가 많다는 것을 의미. "),
        _createElementVNode("br"),
        _createTextVNode(" IQR(Interquartile Range) : 사분위 범위는 데이터를 크기순으로 정렬했을 때, 전체를 4등분한 구간 중 중앙 50%에 해당하는 값의 범위를 말한다. 즉, 제3사분위수(Q3)에서 제1사분위수(Q1)를 뺀 값이다."),
        _createElementVNode("br"),
        _createTextVNode(" Kolmogorov-Smirnov 검정(K-S test) : K-S 검정은 데이터 분포가 특정 이론적 분포(예: 정규분포)와 유사한지를 검정하는 비모수 통계 방법."),
        _createElementVNode("br"),
        _createTextVNode(" p-value(유의 확률) : p-value 는 귀무가설이 참 일때, 관측된 통계량 이상의 극단적인 결과가 나올 확률을 의미. 일반적으로 p-value가 충분히 작으면(예: 0.05보다 작으면) 귀무가설을 기각하고, 관측 결과가 통계적으로 유의미하다고 판단. ")
      ], -1)),
      _createElementVNode("table", _hoisted_32, [
        _cache[33] || (_cache[33] = _createElementVNode("thead", null, [
          _createElementVNode("tr", null, [
            _createElementVNode("th", null, "컬럼"),
            _createElementVNode("th", null, "평균"),
            _createElementVNode("th", null, "표준편차"),
            _createElementVNode("th", null, "최소값"),
            _createElementVNode("th", null, "최대값"),
            _createElementVNode("th", null, "IQR"),
            _createElementVNode("th", null, "p-value")
          ])
        ], -1)),
        _createElementVNode("tbody", null, [
          (_openBlock(true), _createElementBlock(_Fragment, null, _renderList($data.statisticsResult, (stat, col) => {
            return _openBlock(), _createElementBlock("tr", { key: col }, [
              _createElementVNode("td", null, _toDisplayString(col), 1),
              _createElementVNode("td", null, _toDisplayString(stat.mean_orig.toFixed(3)) + " / " + _toDisplayString(stat.mean_synth.toFixed(3)), 1),
              _createElementVNode("td", null, _toDisplayString(stat.std_orig.toFixed(3)) + " / " + _toDisplayString(stat.std_synth.toFixed(3)), 1),
              _createElementVNode("td", null, _toDisplayString(stat.min_orig.toFixed(3)) + " / " + _toDisplayString(stat.min_synth.toFixed(3)), 1),
              _createElementVNode("td", null, _toDisplayString(stat.max_orig.toFixed(3)) + " / " + _toDisplayString(stat.max_synth.toFixed(3)), 1),
              _createElementVNode("td", null, _toDisplayString(stat.iqr_orig.toFixed(3)) + " / " + _toDisplayString(stat.iqr_synth.toFixed(3)), 1),
              _createElementVNode("td", null, _toDisplayString(stat.p_value.toFixed(5)), 1)
            ]);
          }), 128))
        ])
      ]),
      _createElementVNode("button", {
        class: "button-primary",
        onClick: _cache[15] || (_cache[15] = (...args) => $options.downloadStatisticsExcel && $options.downloadStatisticsExcel(...args))
      }, "통계 비교 결과 다운로드")
    ])) : _createCommentVNode("", true),
    $data.error ? (_openBlock(), _createElementBlock("div", _hoisted_33, _toDisplayString($data.error), 1)) : _createCommentVNode("", true)
  ]);
}
const SynthData = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-efd06164"]]);
export {
  SynthData as default
};
